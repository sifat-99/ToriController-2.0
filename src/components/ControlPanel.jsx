  ...
  return (
    <div className="flex flex-col gap-3 sm:gap-4 lg:gap-5 lg:w-64 xl:w-72 w-full bg-black p-3 sm:p-4 lg:border-l lg:border-t-0 border-t border-white/20 text-white shrink-0 lg:h-full lg:overflow-y-auto overflow-y-auto control-panel-gap control-panel-padding">
        ...
        <div className="flex items-center justify-center py-1 sm:py-2 md:py-3 control-panel-throttle-wrapper">
          <div className="relative h-96 w-28 bg-white/5 border border-white/20 rounded-full flex justify-center py-4 shadow-inner control-panel-throttle">
            ...
            <div className="relative w-4 h-full bg-white/10 border border-white/20 rounded-full pointer-events-none shadow-inner">
              <div
                className="absolute bottom-0 w-full bg-white shadow-[0_0_15px_rgba(255,255,255,0.5)] transition-all rounded-b-full"
                style={{ height: `${throttleLimit}%` }}
              />
              ...
              <div
                ...
                style={{
                  bottom: `calc(${throttleLimit}% - var(--throttle-handle-offset, 1.5rem))`,
                }}
              >
            ...
            {/* Markers */}
            <div className="absolute left-1/2 translate-x-10 h-[calc(100%-2rem)] top-4 flex flex-col justify-between text-[11px] font-mono text-white/80 font-bold pointer-events-none">
              <span>MAX</span>
              <span>75</span>
              <span>50</span>
              <span>25</span>
      ...
      <div className="flex flex-col gap-1.5 sm:gap-2 p-2.5 sm:p-3 bg-white/5 border border-white/10 rounded-lg control-panel-card">
        ...
        <div className="text-xs text-white/80 italic mb-1">Use A/D Keys</div>
        <input
          type="range"
          min="-45"
          max="45"
      ...
      <div className="flex flex-col gap-1.5 sm:gap-2 p-2.5 sm:p-3 bg-white/5 border border-white/10 rounded-lg control-panel-card">
        <div className="flex items-center gap-2 text-sm font-semibold mb-2 opacity-95 control-panel-card-title">
          <Move3d size={16} />
          EMPENNAGE (REAR)
        </div>
        ...
        <div className="text-[10px] text-white/80 font-mono mt-1 text-center bg-white/5 py-1 rounded border border-white/10 control-panel-hint">
          Use Arrow Keys (↑↓←→)
        </div>
      </div>
    </div>
